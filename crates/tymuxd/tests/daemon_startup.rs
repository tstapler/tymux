...
/// A short, fresh, not-yet-existing per-test socket directory — deliberately
/// NOT nested under `xdg_state_home` (whose descriptive-label-plus-uuid
/// naming in this file's fixtures can push a nested `.../uds/tymuxd.sock`
/// past `SUN_LEN`, the ~108-byte kernel limit on `AF_UNIX` paths — hit in
/// practice while wiring this up) and NOT `xdg_state_home` itself (already
/// pre-created at the default, non-0700 mode by this file's fixtures;
/// `bind_uds_listener` refuses to bind into a pre-existing directory at
/// the wrong mode, by design).
...
fn spawn_daemon(addr: &str, xdg_state_home: &Path) -> DaemonProcess {
    let child = Command::new(env!("CARGO_BIN_EXE_tymuxd"))
        ...
        // per socket path (ADR-001's flock) — give each spawned daemon its
        // own path so this file's own parallel `#[tokio::test]`s (and
        // other real-subprocess tests in this crate) don't contend for
        // the same uid-scoped default socket.

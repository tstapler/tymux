...
// mirrors tymux-cli's dial_channel shape and examples/list-sessions/main.go's
// identically-shaped dialClient (Epic 7.2, Task 7.2.1b). The probe dial's
// error is classified exactly like dial_channel's fix for pre-mortem.md P1
// #1: syscall.EACCES means a daemon IS listening at socketPath and the
// kernel denied this connect() itself, so it is a hard error that never
// falls back to the unauthenticated TCP path; anything else

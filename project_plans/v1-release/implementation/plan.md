...
2. **Linux ARM64 build path**: native `ubuntu-24.04-arm` GitHub-hosted runner vs. cross-compilation from `ubuntu-latest`. `stack.md` §7 and `pitfalls.md` §7 lean toward native runners but the exact runner label availability/cost should be confirmed at Epic 8 implementation time, not assumed stable from research alone.
...
9. **`StatusBarModel` wire-shape decision**: whether the status-bar-shaped read is folded into the existing `WindowLayoutEvent` stream (Epic 3's `WatchWindow`) or exposed as a new dedicated `GetStatusBar`-style RPC. Epic 6 Story 6.1 Task 1 currently defers this to "implementation-time call, documented either way" — flagged here explicitly per the architecture review so Phase 4 validation resolves it deliberately instead of it being picked ad hoc mid-Epic-6.
...
11. **`RESOLVED` at Epic 3 implementation time — Story 3.4 AC3's concurrency test uses real timing, not a `SlowMockPane` test double.** The plan named a `SlowMockPane`/mock-resize test double (mirroring Story 4.2's `SlowMockPersistenceBackend` pattern) to deterministically prove `ListSessions` on an unrelated session isn't blocked while a window's resize syscalls are in flight. Building that abstraction would mean introducing a `PtyBackend`-style trait solely for this one test, with no other caller — a real abstraction cost for a single assertion. Implemented instead: the lock-release-around-`Pane::resize()` discipline itself (compute geometry under `sessions`/`panes`, release both, then call `Pane::resize()` outside either lock — see `Engine::recompute_window_geometry`), which is the actual correctness property; it is exercised by real `Engine`/daemon integration tests using genuine ptys, not asserted via a mock's timing. If a real regression surfaces in this area post-1.0, that's the trigger to build the mock seam then, not preemptively now.

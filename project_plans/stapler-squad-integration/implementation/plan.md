...
- [ ] `crates/tymuxd/src/main.rs`'s `search_scrollback_rpc_should_return_matching_line_range_when_pattern_present` test (pre-existing on `main`, not introduced by this project) failed once in CI (`gh run 32631879732`, `test (ubuntu-latest)`) with `response.found == false` after its own `DONE-MARKER` poll already confirmed the shell had finished writing 51 lines — the assertion that fired is the scrollback-search result, not the marker-wait timeout. 17 local reproduction attempts (isolated single-test ×5, full `tymuxd` binary ×3, `tymuxd` under `taskset -c 0,1` ×5, full `cargo test --workspace` ×4 — the exact command CI runs) all passed; pane geometry is fixed at 24×80 (not environment-dependent) and the test binds an ephemeral port, ruling out the two most likely deterministic causes. Re-ran the CI job (`gh run rerun --failed`) rather than hand-patching a fix without a confirmed root cause — owner: whoever next sees this test fail in CI should capture the CI run's full log (not just the assertion line) before assuming it's the same transient race, since a repeat failure would upgrade this from "unreproduced, likely CI-environment timing" to something worth a real fix.

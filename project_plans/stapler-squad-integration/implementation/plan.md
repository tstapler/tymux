...
- [ ] Concrete numeric acceptance threshold for the concurrent load test (scale-feasibility.md left this an open planning decision) — blocks Story 1.7.2's pass/fail criterion — owner: this plan sets a starting threshold (p99 `CreateSession` latency stays under 200ms at n=1000 under a 200-way concurrent burst, no `RecvError` disconnects) in Story 1.7.2; revise after the first real measurement if it proves too strict or too loose.

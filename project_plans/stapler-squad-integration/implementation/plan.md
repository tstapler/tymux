...
| `create_session` O(1) fix | New `Engine::session_snapshot(id)` doing a direct `HashMap::get` | PoEAA (Repository — narrow, purpose-built read replacing a full-table-scan-then-filter) | Keep `list_sessions().find()` but memoize/cache it | Caching a snapshot invites staleness bugs (a session created microseconds ago must never read a stale cache); the actual fix is *not scanning at all* for a keyed lookup that's already O(1) by construction (`sessions` is a `HashMap<Uuid, _>`). **Scope note**: this fix covers `session_snapshot` only — `window_snapshot` is explicitly scoped out (Domain Glossary row above), and `window_id_for_pane`/`recompute_window_geometry` retain the same O(n)-scan-under-lock shape without ever having been scoped in or out; see Unresolved Questions. |
...
- [x] **Resolved** — Whether `clients/go/`'s generated package ships as a tagged, versioned module stapler-squad `go get`s, or stays on a `replace` directive indefinitely during co-development — stack.md §3 flagged this as a real open decision, not just a config detail. **Fix landed**: tagged `clients/go/v0.1.0` on tymux's `f56bf5e` (PR #34), then switched stapler-squad's `go.mod` from `replace github.com/tstapler/tymux/clients/go => ../tymux/clients/go` to a real `v0.1.0` dependency (stapler-squad PR #610), removing the 18 CI sibling-checkout sites across 9 workflow files that existed solely to satisfy the `replace` directive's relative path. **Residual, deliberately not automated**: the module isn't wired to any tagging automation (no dependabot/renovate watching it) — every future `clients/go` change still needs a human to cut a new tag and a human to bump stapler-squad's `go.mod`. Flagged by review as a real but non-blocking fast-follow given the current pace of active co-development between the two repos — owner: whoever next needs to change `clients/go` and feels the manual-tag friction firsthand.
...
- [x] **Resolved** — Epic 1.4's O(1) fix covered `session_snapshot` only; two other `Engine` methods retain the identical O(n) `.values().flat_map()/.find()`-under-global-`sessions`-lock shape and were never included in that fix or explicitly scoped out — `window_id_for_pane` (`engine.rs:887-895`) and `recompute_window_geometry` (`engine.rs:795-862`, which does the scan **twice**: once at `engine.rs:812-815` to locate the window to mutate, once more at `engine.rs:851-854` to find the owning session for persistence). This is unlike `window_snapshot` (`engine.rs:897-909`), which the Domain Glossary's `Engine::session_snapshot(id)` row and the Pattern Decisions table both explicitly call out as "stays O(n) and is out of scope here" — no equivalent scoping statement exists anywhere in this plan for these two. Both run on hotter paths than the one that was fixed: `window_id_for_pane` executes once per `Attach` call (`main.rs:649`, i.e. every reconnect, not once per session lifetime like `create_session`), and `recompute_window_geometry` executes on every `Resize` message (via `report_viewport_and_recompute`, `engine.rs:777`) and every client detach (`main.rs:769-770`) — both far more frequent than session creation. Separately, `research/scale-feasibility.md` names `window_id_for_pane` as part of `Attach`'s O(n) cost at MEDIUM confidence, explicitly noting no concurrent-load test was run for it specifically — and Story 1.7.3's mass-reconnect load test (the one that measured the 5576.21ms p99, line 75 above) attributed that latency entirely to "concurrent PTY-spawn/stream-open contention" **by analogy** to Story 1.7.2's isolated-single-call diagnostic (Task 1.7.2a/1.7.2b) — but that diagnostic was run for `CreateSession`/`session_snapshot` specifically, never for `Attach`/`window_id_for_pane`. The causal attribution of Story 1.7.3's p99 is therefore plausible but **unverified** for this specific path, not confirmed. Owner/next step: whoever picks up further scale work on tymux, or as part of a real-hardware validation pass, should either (a) run an isolated single-call diagnostic for `window_id_for_pane` the same way Task 1.7.2a/1.7.2b did for `session_snapshot` (measure isolated latency at n=100 vs n=900 for a single `Attach` call, not under concurrent burst) to confirm or rule out its contribution, or (b) apply `session_snapshot`'s same O(1)-index fix pattern if the diagnostic confirms a real contribution. **Partial diagnostic run for this finding** (throwaway `#[test]` calling `Engine::window_id_for_pane` directly after populating `n` real sessions via `create_session`, timed with `Instant::now()`, matching Task 1.7.2a/1.7.2b's style; not committed — code discarded after the run): single-call, uncontended latency was **~1µs at n=100 vs. ~7-13µs at n=900** across 4 runs (e.g. 2.1µs→7.2µs, 0.9µs→9.7µs, 1.0µs→12.9µs) — confirms the scan cost does grow with session count as expected for an O(n) pattern, but at microsecond scale this is 5-6 orders of magnitude below Story 1.7.3's 5576.21ms measured p99, so it is very unlikely to be a *meaningful* contributor to that latency figure in isolation. This is **not** a full answer, though: it measures single-threaded, uncontended cost only, not lock-contention effects when ~1000 concurrent `Attach` calls all take the same `sessions` lock at once (the actual shape of Story 1.7.3's scenario) — a real concurrent-load diagnostic (mirroring Task 1.7.2b's before/after burst comparison, not just a single isolated call) is still needed to fully rule this path in or out. Recorded here as a real, if partial, data point rather than a purely hypothetical open question. **Fix landed, PR #35** (initial version, then hardened by review — see below): added `WindowIndex`, a small type wrapping two reverse maps (pane_id → window_id, window_id → session_id) behind **one** `Mutex` on `Engine`, maintained at every site that adds/removes a window or pane (`load_persisted`, `create_session`, `split_pane`, `close_pane`, `create_window`, `kill_session`; `revive_session` needs no changes since it respawns panes under their existing ids). `window_id_for_pane`, `recompute_window_geometry`, and (newly, since the index made it trivial) `split_pane`/`close_pane`'s own pane→session resolution now do O(1) lookups instead of scanning every session. Real measurement: `window_id_for_pane` latency is flat — ~700ns-1µs regardless of n=100 vs n=900 sessions — versus the pre-fix ~1µs→7-13µs growth.

**Review caught a real BLOCKER, independently, twice**: the initial version used *two* separate mutexes (`pane_window`, `window_session`), and two of the six call sites acquired them in opposite order (`kill_session`: window_session-then-pane_window; `close_pane`: the reverse, while still holding the first) — a genuine deadlock under concurrent tonic RPC handlers, since `Engine` is `Arc`-shared across request tasks. Two independent review agents found this via direct code inspection (not speculation) and both recommended the same fix: collapse the two maps into one type behind one lock, making the ordering bug structurally impossible rather than a discipline to remember. Implemented as `WindowIndex`.

**Review also caught a real test-quality gap, empirically, not just by inspection**: one agent's own regression test claimed to cover all six mutation sites, but (a) never touched `load_persisted` at all, and (b) for `close_pane`/`kill_session`'s `window_session` cleanup specifically, the agent *deleted the actual cleanup code* and reran the test — it still passed, because `recompute_window_geometry`'s own redundant real-state check silently absorbed the missing removal. Fixed by: adding a `#[cfg(test)]`-only `window_index_len()` accessor so tests can assert on the index's actual size, not just through methods that re-check real state; adding cross-session-isolation assertions after *every* mutation (not just once at the end); adding a `close_pane` "session itself closes" case (a third live session, not just two); and adding a dedicated `load_persisted` coverage test. Re-ran the same ablation (deleted the fixed code's index-removal call, reran the strengthened test) — it now fails, confirming the test genuinely catches the regression this time.

Full workspace test suite after all fixes: 170 passed, 0 failed, 1 known-ignored. `window_snapshot`'s own O(n) scan remains explicitly out of scope, unchanged (see the Domain Glossary/Pattern Decisions scoping this bullet originally referenced).

- [x] **Resolved** — `crates/tymuxd/src/main.rs`'s `search_scrollback_rpc_should_return_matching_line_range_when_pattern_present` test (pre-existing on `main`) failed once in CI (`gh run 32631879732`, `test (ubuntu-latest)`) with `response.found == false`; 17 local reproduction attempts (isolated, full-suite, CPU-constrained, all matching CI's exact command) turned up nothing, so this was first logged here as unreproduced CI timing and the job was re-run rather than guess-fixed. The real root cause surfaced two CI runs later via a second, related failure on macOS: `create_session_should_spawn_pane_in_requested_cwd_and_return_it_on_pane_cwd_field` failed with `got: pwd; echo DONE-MARKER` — the literal *echoed input line*, not command output. Both tests (plus `spawn_shell_with_numbered_lines` in `tymux-core/src/pane.rs`, sharing the same pattern) write a shell command containing the literal completion-marker text, then poll the pane's grid for that same literal string; since a PTY echoes typed input before the shell executes it, the poll can match the echoed command line itself. Fixed in commit a0b8ea0: the marker is now emitted via a separate `echo DONE-MARKER | rev` after the real command, and the poll checks for the reversed string, which can never appear in the typed input. Local repro attempts hadn't hit this race because it needs specific PTY-echo/scheduler timing more common on CI's shared runners than this dev machine.
